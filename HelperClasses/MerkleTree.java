...
public class MerkleTree {
  ...
  public List<Pair<String,String>> QueryDocument(int doc_idx, Transaction[] trarray){
    int dc=0;
    String s = ""+doc_idx;
    for(int i=0; i<trarray.length; i++) 
      if(s.equals(trarray[i].coinID)){
        dc = i+1; break;
      }
		List<Pair<String,String>> li = new ArrayList<Pair<String,String>>();
		TreeNode t = this.rootnode;
		Pair<String,String> p = new Pair<String,String>(t.val, null);
		li.add(p);
		for(int i=numdocs, j=0, k=(i+j)/2 ; j<i-1 ; k=(i+j)/2){
			Pair<String,String> p1 = new Pair<String,String>(t.left.val, t.right.val);
			li.add(p1);
			if(dc<=k){ t = t.left;  i=k; }
			else{ t = t.right;  j=k; }
		}
